...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const menuItems = document.querySelectorAll(
    ".sidebar-menu-item, .bottom-nav .nav-item",
  );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!newCardRadio || !newCardForm || paymentMethods.length === 0) {
    return;
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const form = document.querySelector(
    ".auth-form > div:not(.reset-instructions):not(:last-child)",
  );

  if (!resetBtn || !resetInstructions || !resendBtn || !form) {
    return;
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateTotalPrice() {
    ...
    const totalPriceElement = document.querySelector(
      ".bottom-fixed-bar .fw-bold",
    );
    const seatCountElement = document.querySelector(
      ".bottom-fixed-bar .text-muted.small",
    );

    if (!totalPriceElement || !seatCountElement) {
      return;
    }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!bookForOthersCheckbox || !emergencyContactSection) {
    return;
  }
