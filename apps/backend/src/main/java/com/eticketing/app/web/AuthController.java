...
record PhonePayload(@NotBlank
        String countryCode, @NotBlank
        String number) {
...
record RegisterRequest(
        ...
        @NotNull
        RoleEnum role,
        com.eticketing.app.user.AppEnum app
        ) {
...
public class AuthController {
    ...
    public ResponseEntity<AuthResponse> register(@Valid @org.springframework.web.bind.annotation.RequestBody RegisterRequest request) {
        ...
        com.eticketing.app.user.AppEnum app = request.app() != null ? request.app() : com.eticketing.app.user.AppEnum.FRONT;
        if (hasEmail && users.findByEmailAndApp(request.email(), app).isPresent()) {
        ...
        RoleEnum role = (request.role() == RoleEnum.ADMIN) ? RoleEnum.ADMIN : RoleEnum.CUSTOMER;
        UserType u = new UserType(request.firstName(), request.lastName(), email, phone, encoder.encode(request.password()), role);
        u.setApp(app);
        users.save(u);
        String token = jwt.generateTokenWithApp(u.getId(), u.getRole().name(), app.name(), 3600 * 24);
    ...
    public ResponseEntity<AuthResponse> login(@Valid @org.springframework.web.bind.annotation.RequestBody LoginRequest request) {
        ...
        AppEnum app = request.app() != null ? request.app() : AppEnum.FRONT;

        var userOpt = hasEmail
                ? users.findByEmailAndApp(request.email(), app)
