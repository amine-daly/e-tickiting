...
export class AuthHTTPService {
  ...
  createUser(user: UserType): Observable<any> {
    ...
    return this.http.post<UserType>(API_USERS_URL, user);
  }
  ...
  getUserByToken(token: string): Observable<UserType | undefined> {
    const user = UsersTable.users.find((u: UserType) => {
  ...
  getAllUsers(): Observable<UserType[]> {
    return this.http.get<UserType[]>(API_USERS_URL);
  }
}
